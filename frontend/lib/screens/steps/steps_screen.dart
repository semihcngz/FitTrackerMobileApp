...
class _StepsScreenState extends State<StepsScreen> {
  ...
  Future<void> _load() async {
    final res = await dataService.getStepsToday();
    setState(() {
      s = res;             // <-- s'yi de setState içinde ata
      loading = false;
    });
  }
  ...
  Widget build(BuildContext context) {
    // Güvenli okuma + geriye dönük anahtar desteği
    final num count = (s?['count'] ?? s?['steps'] ?? 0) as num;
    final num goal  = (s?['goal']  ?? 10000) as num;
    final num percentNum = goal == 0 ? 0 : (count / goal);
    final double percent = percentNum.clamp(0, 1).toDouble();
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(children: const [
                          ...
                          SizedBox(width: 8),
                          Text("Today's Steps", style: TextStyle(fontWeight: FontWeight.w600)),
                        ]),
                      ...
                      Row(children: [
                        Expanded(
                          child: OutlinedButton(
                            onPressed: () async { await dataService.addSteps(100); await _load(); },
                        ...
                        Expanded(
                          child: OutlinedButton(
                            onPressed: () async { await dataService.addSteps(500); await _load(); },
                        ...
                        Expanded(
                          child: OutlinedButton(
                            onPressed: () async { await dataService.addSteps(1000); await _load(); },
